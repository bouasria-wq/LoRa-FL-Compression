...
class HomeNode:
    ...
    def train_on_day(self, day_num):
        ...
        self.daily_metrics.append({'day': day_num, 'mae': mae,
    ...
    def run_day(self, day_num):
        ...
        done_flag = self.lora_dir / f'home_{self.home_id:02d}_tx_done_day{day_num}.flag'
        print(f"[HOME {self.home_id:02d}] TX {len(payload)} bytes via USRP B200...")
        result = self.radio.transmit(payload, done_flag_path=str(done_flag))
