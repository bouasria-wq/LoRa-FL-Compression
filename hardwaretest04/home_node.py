"""
Individual Home Node - ME-CFL USRP Version (hardwaretest04)
=============================================================
Same FL pipeline as hardwaretest03, but LoRa TX/RX goes through
REAL USRP B200 hardware over the air instead of simulated channel.

Uses the REAL EPFL lora_TX.grc and lora_RX.grc flowgraphs.
Two USRP B200 radios: one for TX, one for RX.
...
class HomeNode:
    ...
    def __init__(self, home_id, n_days=7, epochs_per_day=100,
        ...
        print(f"\nHOME {self.home_id:02d} - ME-CFL USRP VERSION (hardwaretest04)")
    ...
    def train_on_day(self, day_num):
        print(f"\n--- HOME {self.home_id:02d} | DAY {day_num} | "
              f"{self.epochs_per_day} EPOCHS | ME-CFL USRP ---")
    ...
    def transmit_via_lora(self, payload: bytes) -> dict:
        """Transmit Hegazy compressed model via USRP B200 over the air."""
    ...
    def wait_for_global_model(self, day_num, timeout=120):
        """
        Wait for server to broadcast global model.
        Uses file-based signaling (server writes global_model.bin).
        """
    ...
    def run_day(self, day_num):
        ...
        # Transmit via USRP B200 over the air
    ...
    def run(self):
        ...
        print("\n" + "="*50)
        print(f"FINAL HOME {self.home_id:02d} SUMMARY - ME-CFL USRP")
        print("="*50)
...
def main():
    ...
    parser.add_argument('--tx_serial', type=str, default=None,
                        help='USRP B200 serial for TX (default: 32BBAD0)')
    parser.add_argument('--rx_serial', type=str, default=None,
                        help='USRP B200 serial for RX (default: 32BBA90)')
